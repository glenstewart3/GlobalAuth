...
export default function Login() {
  ...
  // Google sign-in — receives a signed ID token from Google directly
  const handleGoogleSuccess = async ({ credential }) => {
    ...
    try {
      ...
      // Restore full auth state from the refresh cookie that was just set
      await refreshAuth();
      if (appRedirect) {
        window.location.href = `${appRedirect}#mps_token=${token}`;
      } else {
        toast.success("Welcome back!");
        navigate("/dashboard", { replace: true });
      }
      ...
      const detail = err.response?.data?.detail;
      setError(
        typeof detail === "string" ? detail : "Google sign-in failed. Make sure your Google account is linked to an MPS Auth account."
      );
    }
  };
    ...
    <div className="min-h-screen flex">
      <div className="flex-1 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md">
          ...
          <div className="mb-4" data-testid="google-login-button">
            <GoogleLogin
